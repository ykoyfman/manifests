...
func writeNotebookControllerOverlaysOpenshift(th *KustTestHarness) {
	th.writeF("/manifests/jupyter/notebook-controller/overlays/openshift/addfsgroup-env.yaml", `
apiVersion: apps/v1
kind: Deployment
metadata:
  name: deployment
        ...
        - name: manager
          env:
            - name: ADD_FSGROUP
              value: "false"
`)
	th.writeK("/manifests/jupyter/notebook-controller/overlays/openshift", `
...
func TestNotebookControllerOverlaysOpenshift(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/jupyter/notebook-controller/overlays/openshift")
	writeNotebookControllerOverlaysOpenshift(th)
	...
	targetPath := "../jupyter/notebook-controller/overlays/openshift"
	fsys := fs.MakeRealFS()
